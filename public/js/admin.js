...
function sendMessage(id) {
  const inputMessage = document.getElementById(`send_message_${id}`)
  ...
  createDiv.className = 'admin_message_admin'
  createDiv.innerHTML = `<span>Atendente:</span>`
  createDiv.innerHTML += `<span>${inputMessage.value}</span>`
...
socket.on('admin_receive_message', data => {
  const connection = userConnections.find(connection => connection.socket_id === data.socket_id)
